;(function($){
	$.www_upload = function(options){
		...
		this.html = function(rs,i){
			...
			html += '	<input type="button" value="预览" class="btn btn-xs blue" onclick="obj_'+opts.id+'.preview(\''+rs.id+'\')" />';
			html += '	<input type="button" value="删除" class="btn btn-xs btn-danger" onclick="obj_'+opts.id+'.del(\''+rs.id+'\')" /></div>';
			html += '</div></div>';
			html += '</div>';
			return html;
		};
