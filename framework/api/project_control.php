...
class project_control extends dsy_control
{
	...
	public function index_f()
	{
		...
		$project = $this->call->dsy('_project',array('pid'=>$pid));
	...
	private function load_module($rs,$parent_rs='')
	{
		...
		if($rs["cate"]){
			$cate = $this->get("cate");
			if($cate){
				$cate_rs = $this->call->dsy('_cate',array('pid'=>$rs['id'],'cate'=>$cate));
		...
		if($cate_root){
			if($cateid && $cateid != $cate_root){
				$cate_rs = $this->call->dsy('_cate',array('pid'=>$rs['id'],'cateid'=>$cateid));
				...
				if($cate_rs['parent_id'] && $cate_rs['parent_id'] != $cate_root){
					$cate_parent_rs = $this->call->dsy('_cate',array('pid'=>$rs['id'],'cateid'=>$cate_rs['parent_id']));
			...
			$cate_root_rs = $this->call->dsy('_cate',array('pid'=>$rs['id'],'cateid'=>$cate_root));
		...
		$dt['fields'] = $fields;
		$info = $this->call->dsy('_arclist',$dt);
		unset($dt);
		if(!$info['rslist']){
			...
			foreach($info['rslist'] as $key=>$value){
				foreach($value as $k=>$v){
					if($funclist[$k] && $v && $funclist[$k][0]== 'cut'){
						if($k == 'title'){
							$value['_title'] = dsy_cut($v,$funclist[$k][1],($funclist[$k][2] ? '…' : ''));
						}else{
							$value[$k] = dsy_cut($v,$funclist[$k][1],($funclist[$k][2] ? '…' : ''));
						}
