...
class tag_control extends dsy_control
{
	...
	function index_f()
	{
		...
		foreach($idlist AS $key=>$value){
			if(substr($value['id'],0,1) == 'p'){
				...
				$rslist[] = $this->call->dsy('_project',array('pid'=>$tmp));
				...
				$rslist[] = $this->call->dsy('_cate',array('pid'=>$project_info['id'],'cateid'=>$tmp));
			}else{
				$rslist[] = $this->call->dsy('_arc',array('title_id'=>$value['id']));
			}
		}
