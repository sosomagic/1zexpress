...
/***********************************************************
	...
	Update  : 2016-05-16 16:10
***********************************************************/
if(!defined("DSAIYIN_SET")){exit("<h1>Access Denied</h1>");}
class open_control extends dsy_control
{
	...
	public function index_f()
	{
		...
		$this->view($this->dir_dsy.'open/res_openselect.html','abs-file');
	}
	...
	public function input_f()
	{
		...
		$this->assign("id",$id);
		$this->view($this->dir_dsy.'view/'.$tplfile.'.html','abs-file');
	}
	...
	function user2_f()
	{
		...
		if(!$psize) $psize = 30;
		$keywords = $this->get("keywords");
		$pageurl = $this->url("open","user2","id=".$id);
		$condition = "1=1";
		if($keywords){
			$this->assign("keywords",$keywords);
			$condition .= " AND u.user LIKE '%".$keywords."%'";
    ...
    function address_f()
    {
        ...
        if($pca_a){
            $condition = " AND county ='".$pca_a."'";
        }
        $rslist = $this->model('user')->address($_SESSION['user_id'],$condition);
        if($rslist){
            $this->assign('rslist',$rslist);
        }*/
    ...
    function sender_f()
    {
        ...
        $rslist = $this->model('user')->sender($_SESSION['user_id'],$condition);
        if($rslist){
            $this->assign('rslist',$rslist);
        }
        $this->assign("id",$id);
        $this->view("open_sender_list");
    }
}
?>