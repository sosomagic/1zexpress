...
function end_param()
{
	...
	$.dsy.json(url,function(rs){
		if(rs.status == 'ok'){
			...
			html += '<input type="button" value="全部字段" onclick="input_fields(\'*\')" class="dsy-btn" />';
			var lst = rs.content.rslist;
			for(var i in lst){
				html += '<input type="button" value="'+lst[i].title+'" onclick="input_fields(\''+lst[i].identifier+'\')" class="dsy-btn" />'
			}
			html += '</div>';
