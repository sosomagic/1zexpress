...
class text_form extends _init_auto
{
	...
	{
		$this->view($this->dir_dsy.'form/html/text_admin.html','abs-file');
	}
	...
	private function _format_admin($rs)
	{
		...
		$this->assign('_rs',$rs);
		$file = $this->dir_dsy."form/html/text_admin_tpl.html";
	...
	private function _format_default($rs)
	{
		...
		$rs['form_style'] = $this->lib('common')->css_format($css);
		$this->assign("_rs",$rs);
		return $this->fetch($this->dir_dsy."form/html/text_www_tpl.html",'abs-file');
	}
}
?>