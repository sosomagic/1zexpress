...
class select_form extends _init_auto
{
	...
	public function dsy_config()
	{
		...
		$this->view($this->dir_dsy.'form/html/select_admin.html','abs-file');
	}
	...
	public function dsy_format($rs,$appid="admin")
	{
		...
		$file = $appid == 'admin' ? $this->dir_dsy.'form/html/select_admin_tpl.html' : $this->dir_dsy.'form/html/select_www_tpl.html';
		if(!is_file($file)){
			$file = $this->dir_dsy.'form/html/select_admin_tpl.html';
		}
		...
		if($appid == 'admin'){
			...
			if($ext['is_multiple']){
				...
				foreach($info as $key=>$value){
					$tmp = $value;
					if($opt[0] == 'project'){
						$tmp = $this->call->dsy('_project',array('pid'=>$value));
					}
					if($opt[0] == 'cate'){
						$tmp = $this->call->dsy('_cate',array('cateid'=>$value));
					}
					if($opt[0] == 'title'){
						$tmp = $this->call->dsy('_arc',array('title_id'=>$value));
					}
					if($opt[0] == 'opt'){
				...
				$tmp = $rs['content'];
				if($opt[0] == 'project'){
					$tmp = $this->call->dsy('_project',array('pid'=>$rs['content']));
				}
				if($opt[0] == 'cate'){
					$tmp = $this->call->dsy('_cate',array('cateid'=>$rs['content']));
				}
				if($opt[0] == 'title'){
					$tmp = $this->call->dsy('_arc',array('title_id'=>$rs['content']));
				}
				if($opt[0] == 'opt'){
