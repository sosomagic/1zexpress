...
class wealth_model_base extends dsy_model
{
	/**
	 * 构造函数
	**/
	...
	**/
	public function recharge($logid)
	{
        ...
        //如果充值成功
        if($ext['dsy_status'] || !$ext['goal']){
            return true;
        }
        $price = price_format_val($order['price'],$order['currency_id'],$this->site['currency_id']);
        $val = $this->model('wealth')->get_val($order['user_id'],2);
        $data = array('wid'=>'2','uid'=>$order['user_id'],'lasttime'=>$this->time);
        $data['val'] = round(($val+$price),2);
        $this->model('wealth')->save_info($data);
        $ext['dsy_status'] = true;
        $tmp = serialize($ext);
        $this->model('payment')->log_update(array('ext'=>$tmp,'balance'=>$data['val']),$logid);
        return true;
	}
