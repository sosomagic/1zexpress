<?php
/*****************************************************************************************
	文件： /model/admin/wealth_model.php
	备注： 财富管理
	版本： 2.x
	网站： www.dsaiyin.com
	作者： dsaiyin <QQ:17189095>
	时间： 2016年05月22日 00时05分
*****************************************************************************************/
...
class wealth_model extends wealth_model_base
{
    ...
    //扣款
    /*function update_wealth($val,$uid,$wid){
        $rs = $this->model("wealth")->get_val($uid,$wid);//银行zhuan'z
		if($rs){
			$sql = "UPDATE ".$this->db->prefix."wealth_info SET val='".$val."',lasttime='".$this->time."' ";
			$sql.= "WHERE uid='".$uid."' AND wid='".$wid."'";
		}else{
			$sql = "INSERT INTO ".$this->db->prefix."wealth_info (wid,uid,lasttime,val) VALUES ('".$wid."','".$uid."','".$this->time."','".$val."')";
		}
        $this->db->query($sql);
        return true;
    }*/
