...
class data_model_base extends dsy_model
{
	...
	{
		if(!$rs['pid'] && !$rs['dsy']) return false;
		if(!$rs['pid'])
		{
			$tmp = $this->_id($rs['dsy'],$this->site['id']);
	...
	{
		if(!$rs['cateid'] && !$rs['dsy']) return false;
		if(!$rs['cateid'])
		{
			$tmp = $this->_id($rs['dsy'],$this->site['id']);
