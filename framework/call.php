...
class dsy_call extends dsy_control
{
	private $mlist;
	private $dsylist;
	...
	private function load_dsylist($id,$siteid=0)
	{
		if($this->dsylist && $this->dsylist[$id] && $this->dsylist[$id]['site_id'] == $siteid){
			return $this->dsylist[$id];
		}
		$sql = "SELECT * FROM ".$this->db->prefix."dsycall WHERE site_id='".$siteid."' AND status=1";
		$cache_id = $this->cache->id($sql);
		$this->db->cache_set($cache_id);
		$this->dsylist = $this->cache->get($cache_id);
		if(!$this->dsylist){
			$this->dsylist = $this->model('call')->all($siteid,'identifier');
			if(!$this->dsylist){
				return false;
			}
			if($this->dsylist && $cache_id){
				$this->cache->save($cache_id,$this->dsylist);
			}
		}
		$info = $this->dsylist[$id];
		if(!$info){
			return false;
		}
		return $info;
	...
	private function _dsy_sys_func()
	{
	...
	public function dsy($id,$rs="")
	{
		...
		if(substr($id,0,1) != '_'){
			$call_rs = $this->load_dsylist($id,$rs['site']);
			if(!$call_rs){
				return false;
			}
			if($rs && is_array($rs)){
	...
	private function _arclist($rs,$cache_id='')
	{
		...
		if(!$rs['pid']){
			$tmp = $this->model('id')->id($rs['dsy'],$rs['site'],true);
	...
	{
		$tmpid = $param['dsy'] ? $param['dsy'] : ($param['title_id'] ? $param['title_id'] : $param['id']);
		if(!$tmpid){
			return false;
		}
	...
	{
		if(!$rs['cateid'] && !$rs['dsy'] && !$rs['cate']){
			return false;
		}
		if(!$rs['cateid']){
			$identifier = $rs['cate'] ? $rs['cate'] : $rs['dsy'];
	...
	{
		if(!$rs['fileids'] && !$rs['dsy']){
			return false;
		}
		if(!$rs['fileids']) $rs['fileids'] = $rs['dsy'];
	...
	{
		if(!$rs['dsy'] && !$rs['user_id']){
			return false;
		}
		$user_id = $rs['user_id'] ? $rs['user_id'] : $rs['dsy'];
