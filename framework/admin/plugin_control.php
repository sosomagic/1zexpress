...
class plugin_control extends dsy_control
{
	private $popedom;
	...
	public function config_f()
	{
        ...
        if($id=='weixin'){
            if(!$this->popedom["wechat"]){
                error(P_Lang('您没有权限执行此操作'),'','error');
            }
        }else{
            if(!$this->popedom["loginext"]){
                error(P_Lang('您没有权限执行此操作'),'','error');
            }
        }
		$this->assign("id",$id);
    ...
    public function save_f()
    {
        ...
        error(P_Lang('{title}设置成功',array('title'=>' <span class="red">'.$rs['title'].'</span> ')),$this->url("plugin","config","id=".$id),'ok');
    }
	...
	public function create_f()
	{
		...
		if($id){
			...
			$id = md5($title.'-dsaiyin.com-'.uniqid(rand(), true));
		}
		//检测插件文件夹是否存在
		if(file_exists($this->dir_root.'plugins/'.$id)){
	...
	{
		$string = 'class '.$fileid.'_'.$id.' extends dsy_plugin'."\n";
		$string.= '{'."\n\t";
		$string.= 'public $me;'."\n\t";
		...
		if($fileid == 'www' || $fileid == 'admin' || $fileid == 'api'){
			...
			$string .= 'public function dsy_before()'."\n\t";
			$string.= '{'."\n\t\t";
			$string.= '//编写代码;'."\n\t";
			$string.= '}'."\n\t";
			$string .= '//全局运行插件，在执行当前方法运行后，数据未输出前'."\n\t";
			$string .= 'public function dsy_after()'."\n\t";
			$string.= '{'."\n\t\t";
			$string.= '//编写代码;'."\n\t";
			$string.= '}'."\n\t";
		}
