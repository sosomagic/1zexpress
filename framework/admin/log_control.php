...
class log_control extends dsy_control
{
    ...
    public function index_f()
    {
        ...
        $adminer = trim($this->get('adminer'));
        if($adminer){
            $pageurl .= "&adminer=".rawurlencode($adminer);
            $this->assign('adminer',$adminer);
            $condition .= " AND admin_id in(SELECT id FROM ".$this->db->prefix."adm WHERE account like '%".$adminer."%')";
        }
        $user = trim($this->get('user'));
        if($user){
            $pageurl .= "&user=".rawurlencode($user);
            $this->assign('user',$user);
            $condition .= " AND user_id in(SELECT id FROM ".$this->db->prefix."user WHERE user like '%".$user."%')";
        }
