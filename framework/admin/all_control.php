...
class all_control extends dsy_control
{
	private $popedom;
	...
	public function save_f()
	{
        ...
        if($currency_id){
           ...
           $this->model('wealth')->save(array('unit'=>$currency['title']),2);
        }else{
            error(P_Lang('请选择默认货币'),$error_url,"error");
        }
		$array = array();
		$array["title"] = $title;
		$array["dir"] = $dir;
	...
	private function all_check($identifier,$id=0)
	{
		...
		if($identifier == "dsy" || $identifier == "config"){
			return "系统标识串：config不允许用户自定义使用";
		}
