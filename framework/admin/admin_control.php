...
class admin_control extends dsy_control
{
	private $popedom;
	...
	public function index_f()
	{
        ...
        foreach($rslist as $key=>$value){
            //if($value['if_system']) continue;//系统账号退出，要不然出错
            if($value['stock_id']){
	...
	public function set_f()
	{
		...
		if($id){
			...
			if($id == $_SESSION["admin_id"] && $_SESSION["admin_id"]!=1){
	...
	{
		$id = $this->get("id","int");
		if($id && $id == $_SESSION["admin_id"] && $_SESSION["admin_id"]!=1 ){
			error(P_Lang('您不能操作自己的信息'),$this->url("admin"),"error");
		}
		if($id){
			if(!$this->popedom["modify"]){
        ...
        //保存仓库ID，要不然每次要重新登陆
        $_SESSION["admin_stock"] = $array['stock_id'];
		$is_edit = false;
		if($id){
