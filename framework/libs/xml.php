...
class xml_lib
{
	private $xml_read_func = 'dsy';
	private $xml_save_func = 'dsy';
	public function __construct()
	{
		if(function_exists('simplexml_load_string')){
			$this->xml_read_func = 'simplexml';
		}
		$this->xml_save_func = 'dsy';
	}

	public function read_setting($type='dsy')
	{
		$this->xml_read_func = $type;
	}

	public function save_setting($type="dsy")
	{
	...
	{
		$this->xml_read_func = function_exists('simplexml_load_string') ? 'simplexml' : 'dsy';
		$this->xml_save_func = 'dsy';
	}
	...
	private function write_dsy($data,$file,$ekey='')
	{
	...
	//通过人工编写自己读取XML，效率较慢
	private function read_dsy($info,$isfile=true)
	{
		if($isfile){
			$info = file_get_contents($info);
		}
	...
	private function _string_to_array($ext='')
	{
		...
		$ext = preg_replace("/[\"|'](^[\"|'|\s]+)\s+(^[\"|'|\s]+)[\"|']/isU",'\\1:_:_:-dsy-:_:_:\\2',$ext);
		...
		foreach($list as $key=>$value){
			...
			$value = str_replace(':_:_:-dsy-:_:_:',' ',$value);
			$list[$key] = $value;
		}
		return $list;
	}
