<?php
/**
 * 分页类
 */
if(!defined("DSAIYIN_SET")){
	exit("<h1>Access Denied</h1>");
}
class page_lib
{
	private $half = 5;
	private $psize = 20;
	private $pageid = "pageid";
	private $num = 1;
	private $always = 0;
	private $add_up = "";
	private $home_str = "";
	private $next_str = "";
	private $prev_str = "";
	private $last_str = "";
	private $opt_str = "";
	private $url_format = '';
	private $url_cut = '';

	public function __construct()
	{
		$this->half = 5;
		$this->psize = 20;
		$this->pageid= "pageid";
		$this->num = 1;
		$this->total = 0;
	}

	public function pageid($pageid)
	{
		$this->pageid = $pageid;
	}

	public function num($num=1)
	{
		$this->num = $num;
	}

	public function half($half=5)
	{
		$this->half = $half;
	}

	public function psize($psize=20)
	{
		$this->psize = $psize;
	}

	public function always($always=0)
	{
		$this->always = $always;
	}

	public function add_up($add_up="")
	{
		$this->add_up = $add_up;
	}

	public function home_str($title)
	{
		$this->home_str = $title;
	}

	public function prev_str($title)
	{
		$this->prev_str = $title;
	}

	public function next_str($title)
	{
		$this->next_str = $title;
	}

	public function last_str($title)
	{
		$this->last_str = $title;
	}

	public function opt_str($title)
	{
		$this->opt_str = $title;
	}

	public function url_format($str='')
	{
		if($str){
			$list = explode("=",$str);
			if($list[1]){
				$this->url_format = $list[1];
				$this->url_cut = $list[0];
			}else{
				$this->url_format = $list[0];
			}
		}
	}

	private function _page_rewrite($url,$total,$num=0,$psize=0,$total_page=0)
	{
		$tmpurl = $this->url_cut ? str_replace($this->url_cut,'',$url) : $url;
		$old = array("(total)","(psize)","(total_page)","(num)","(pageid)","(url)");
		$new = array($total,$psize,$total_page,$num,$this->pageid,$url);
		$dsy_i = 0;
		$list = array();
		if($this->add_up){
			$this->add_up = str_replace($old,$new,$this->add_up);
			$list[$dsy_i]["url"] = "";
			$list[$dsy_i]["title"] = $this->add_up;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "add";
			$list[$dsy_i]['nolink'] = true;
			$dsy_i++;
		}
		if($this->home_str && ($this->always || ($num>1))){
			$list[$dsy_i]["url"] = $url;
			$list[$dsy_i]["title"] = $this->home_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "home";
			if($num <= 1){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		if($this->prev_str && ($this->always || $num > 1)){
			if($num>1){
				$prev_num = $num-1;
				$prev_url = $prev_num > 1 ? $tmpurl.str_replace('(page)',$prev_num,$this->url_format) : $url;
			}else{
				$prev_num = 1;
				$prev_url = $url;
			}
			$list[$dsy_i]["url"] = $prev_url;
			$list[$dsy_i]["title"] = $this->prev_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "prev";
			if($num <= 1){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}

		if($this->half){
			for($i=$num-$this->half,$i>0 || $i=0, $j=$num+$this->half,$j<$total_page || $j=$total_page;$i<$j;$i++){
				$l = $i + 1;
				if($l != 1){
					$purl = $tmpurl.str_replace('(page)',$l,$this->url_format);
				}else{
					$purl = $url;
				}
				$list[$dsy_i]["url"] = $purl;
				$list[$dsy_i]["title"] = $l;
				$list[$dsy_i]["status"] = ($l == $num) ? 1 : 0;
				$list[$dsy_i]["type"] = "num";
				$dsy_i++;
			}
		}
		if($this->next_str && ($this->always || ($total_page > 1 && $num < $total_page))){
			$mynum = $num + 1;
			if($mynum > $total_page){
				$mynum = $total_page;
			}
			$purl = $mynum > 1 ? $tmpurl.str_replace('(page)',$mynum,$this->url_format) : $url;			
			$list[$dsy_i]["url"] = $purl;
			$list[$dsy_i]["title"] = $this->next_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "next";
			if($num >= $total_page){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		if($this->last_str && ($this->always || ($total_page>1 && $num < $total_page))){
			$purl = $total_page > 1 ? $tmpurl.str_replace('(page)',$total_page,$this->url_format) : $url;
			$list[$dsy_i]["url"] = $purl;
			$list[$dsy_i]["title"] = $this->last_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "last";
			if($num >= $total_page){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		return $list;
	}

	public function page($url,$total,$num=0,$psize=0)
	{
		if($num){
			$this->num = $num;
		}
		if($psize){
			$this->psize = $psize;
		}
		if(!$url || !$total){
			return false;
		}
		$total_page = intval($total/$this->psize);
		if($total % $this->psize){
			$total_page ++;
		}
		if($this->num > $total_page){
			$this->num = $total_page;
		}
		if($this->url_format && strpos($url,'?') !== true){
			return $this->_page_rewrite($url,$total,$this->num,$this->psize,$total_page);
		}
		$url = str_replace("&amp;","&",$url);
		if(strpos($url,"?") === false){
			$url .= "?";
		}
		if(substr($url,-1) != "&" && substr($url,-1) != "?"){
			$url .= "&";
		}
		$old_array = array("(total)","(psize)","(total_page)","(num)","(pageid)","(url)");
		$new_array = array($total,$this->psize,$total_page,$this->num,$this->pageid,$url);
		$dsy_i = 0;
		$list = array();
		if($this->add_up){
			$this->add_up = str_replace($old_array,$new_array,$this->add_up);
			$list[$dsy_i]["url"] = "";
			$list[$dsy_i]["title"] = $this->add_up;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "add";
			$dsy_i++;
		}
		if($this->home_str && ($this->always || $this->num>1)){
			$list[$dsy_i]["url"] = substr($url,0,-1);
			$list[$dsy_i]["title"] = $this->home_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "home";
			if($this->num <= 1){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		if($this->prev_str && ($this->always || ($this->num>1))){
			$purl = substr($url,0,-1);
			if($this->num > 1){
				$prev_num = $this->num - 1;
				$purl = $url.$this->pageid."=".$prev_num;
			}
			$list[$dsy_i]["url"] = $purl;
			$list[$dsy_i]["title"] = $this->prev_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "prev";
			if($this->num <= 1){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		if($this->half){
			for($i=$this->num - $this->half,$i>0 || $i=0,$j=$this->num + $this->half,$j<$total_page || $j=$total_page;$i<$j;$i++){
				$l = $i + 1;
				$purl = substr($url,0,-1);
				if($l != 1){
					$purl = $url.$this->pageid."=".$l;
				}
				$list[$dsy_i]["url"] = $purl;
				$list[$dsy_i]["title"] = $l;
				$list[$dsy_i]["status"] = ($l == $this->num) ? 1 : 0;
				$list[$dsy_i]["type"] = "num";
				$dsy_i++;
			}
		}
		if($this->next_str && ($this->always || ($total_page > 1 || $this->num < $total_page))){
			$mynum = $this->num + 1;
			if($mynum > $total_page){
				$mynum = $total_page;
			}
			$purl = $url.$this->pageid."=".$mynum;
			$list[$dsy_i]["url"] = $purl;
			$list[$dsy_i]["title"] = $this->next_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "next";
			if($this->num >= $total_page){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		if($this->last_str && ($this->always || ($total_page>1 && $this->num < $total_page))){
			$purl = $url.$this->pageid."=".$total_page;
			$list[$dsy_i]["url"] = $purl;
			$list[$dsy_i]["title"] = $this->last_str;
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] = "last";
			if($this->num >= $total_page){
				$list[$dsy_i]['nolink'] = true;
			}
			$dsy_i++;
		}
		if($this->opt_str && $this->half){
			$list[$dsy_i]["url"] = $url.$this->pageid."=";
			$list[$dsy_i]["status"] = 0;
			$list[$dsy_i]["type"] ="opt";
			$array = array();
			for($i = $this->num - $this->half * 3,$i>0 || $i=0,$j=$this->num + $this->half * 3,$j<$total_page || $j=$total_page;$i<$j;$i++){
				$l = $i + 1;
				$tmp = array();
				$tmp["value"] = $l;
				$tmp["title"] = str_replace("(num)",$l,$this->opt_str);
				if($tmp['title']){
					$tmp["title"] = str_replace($old_array,$new_array,$tmp['title']);
				}
				$tmp["status"] = $l == $this->num ? 1 : 0;
				$array[] = $tmp;
			}
			$list[$dsy_i]["title"] = $array;
			$dsy_i++;
		}
		return $list;
	}
}
?>