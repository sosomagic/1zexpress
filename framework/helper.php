...
function dsy_cut($string,$length=255,$dot="")
{
...
function error_open($tips,$type="notice",$btn="")
{
	...
	$GLOBALS['app']->view($GLOBALS['app']->dir_dsy."view/tips_open.html","abs-file");
	exit;
}
...
//内容图片本地化操作
function dsy_img_local($content)
{
...
function dsy_filesize($size,$is_file=true)
{
...
function dsy_user_login($id,$pass="",$field='id')
{
	if(!$id){
...
//取得扩展表里的信息
function dsy_ext_info($module,$extc=true)
{
...
//取得主题扩展内容信息
function dsy_ext_list($mid,$tid=0)
{
...
function tpl_head($array=array())
{
	...
	if($app->license == 'LGPL'){
		$html .= '<meta name="author" content="dsaiyin.com" />'."\n\t";
	}
	...
	$html .= '<title>'.trim($headtitle).'- Powered by DsyExpress!</title>'."\n\t";
	if(substr($app->url,-1) != '/'){
		$app->url .= "/";
	}
	...
	if(!$array['close'] || $array["close"] != 'false'){
		$html .= $app->plugin_html_ap("dsyhead");
		$html .= "\n".'</head>';
	}
	$html .= "\n";
	return $html;	
...
//content，表单默认值内容
function form_html($type='text',$id='dsy',$attr='',$content='')
{
...
//日志存储，可用于调试
function dsy_log($info='')
{
	if(!$info){
		$info = '执行 {dsy}/'.$GLOBALS['app']->app_id.'/'.$GLOBALS['app']->ctrl.'_control.php 方法：'.$GLOBALS['app']->func.'_f';
	}
...
//邮箱合法性验证
function dsy_check_email($email)
{
...
function dsy_call_api_url($dsy,$param='',$tpl='')
{
	if(!$dsy || !$GLOBALS['app']->site['api_code']){
		return false;
	}
	$ext = $tpl ? 'tpl='.rawurlencode($tpl) : '';
	$info = array('id'=>$dsy,'param'=>$param);
	$token = $GLOBALS['app']->lib('token')->encode($info);
	if($ext){
		$ext .= "&";
	}
	$ext .= "token=".rawurlencode($token);
	return api_url('index','dsy',$ext,true);
}
