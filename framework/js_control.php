...
/***********************************************************
	...
	Update  : 2016-05-29 20:22
***********************************************************/
if(!defined("DSAIYIN_SET")){exit("<h1>Access Denied</h1>");}
class js_control extends dsy_control
{
	...
	{
		$this->js_base();
		echo $this->lib('file')->cat($this->dir_dsy."form.js");
		echo "\n";
		$ext = $this->get("ext");
		$_ext = $this->get('_ext');
		...
		foreach($list AS $key=>$value){
			...
			$jsfile = is_file($this->dir_root."js/".$value) ? $this->dir_root."js/".$value : $this->dir_dsy."js/".$value;
	...
	public function mini_f()
	{
		...
		if($ext){
			...
			foreach($list AS $key=>$value){
				...
				if(!$value) continue;
				if(strtolower(substr($value,-3)) != '.js') $value .= '.js';
				$file = is_file($this->dir_dsy.'js/'.$value) ? $this->dir_dsy.'js/'.$value : $this->dir_root."js/".$value;
				if(is_file($file)){
					echo "\n";
