<?php
/***********************************************************
	Filename: dsy_tpl_helper.php
	Note	: 模板中常用的函数，此函数在action前才加载
	Version : 2.0
	Web		: www.dsaiyin.com
...
function dsy($id='',$ext="")
{
	...
	$count = func_num_args();
	if($count<=2){
		return $GLOBALS['app']->call->dsy($id,$ext);
	}
	$param = array();
	for($i=1;$i<$count;++$i){
		$tmp = func_get_arg($i);
	...
	$param = implode("&",$param);
	return $GLOBALS['app']->call->dsy($id,$param);
}
 ...
 * @date 2016年02月05日
 */
function dsy_page($url,$total,$num=0,$psize=20)
{
	if(!$url || !$total){
		return false;
	}
...
//前台取得地址表单
function dsy_address($format=false)
{
	...
	if($shipping)
	{
		$flist = $GLOBALS['app']->call->dsy('_fields',array("pid"=>$shipping,'fields_format'=>$format,'prefix'=>"s_"));
		$rs['shipping'] = $flist;
	}
	if($billing)
	{
		$flist = $GLOBALS['app']->call->dsy('_fields',array("pid"=>$billing,'fields_format'=>$format,'prefix'=>'b_'));
		$rs['billing'] = $flist;
	}
	return $rs;
}
...
//读取会员信息
function dsy_user($id)
{
	return $GLOBALS['app']->model('user')->get_one($id);
}

//读取文本内容，并格式化文本内容
function dsy_txt($file,$pageid=0,$type='txt')
{
