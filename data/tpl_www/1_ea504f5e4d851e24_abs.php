<?php if(!defined("DSAIYIN_SET")){exit("<h1>Access Denied</h1>");} ?><link rel="stylesheet" type="text/css" href="js/webuploader/webuploader.css" />
...
$(document).ready(function(){
	...
	obj_<?php echo $_rs['identifier'];?> = new $.www_upload({
		...
		'server':'<?php echo dsy_url(array('ctrl'=>'upload','func'=>'save','cateid'=>$_rs['cate_id']));?>',
...
$(document).ready(function(){
		...
		var url = "<?php echo dsy_url(array('ctrl'=>'open','id'=>$_rs['identifier'],'multiple'=>$_rs['is_multiple'],'cate_id'=>$_rs['upload_type']['id']));?>";
		url = $.dsy.nocache(url);
		var t = "<?php echo $_rs['is_multiple'] ? 'true' : 'false';?>";
		var old = $("#<?php echo $_rs['identifier'];?>").val();
		if(t == 'true'){
			if(old){
				$.cookie.set('dsy_input_<?php echo $_rs['identifier'];?>',old);
			}
			$.dialog.open(url,{
				title: "资源管理器",
				lock : true,
